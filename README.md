...
b) `url_for`: Allows you to make static, relative references to endpoints inside your Python application while it is being served. This is very powerful when incorporating CSS or JavaScript files.
c) `session`: Allows you to store information specific to a user from one request to the next.
d) `redirect`: Relocates the user to a new endpoint inside your Python application.
e) `render_template`: Allows you to send data stored in variables from your Python code to a specified HTML file.
